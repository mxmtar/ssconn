...
struct tcp_control_client {
    ...
    uint8_t             recv_buf[0x10000];
    size_t              recv_length;
    size_t              recv_wait;

    uint8_t             xmit_buf[0x10000];
    size_t              xmit_length;

    struct tcp_control_client_flags {
        unsigned int close:1;
    } flags;

    struct tcp_control_client *next; // list entry
};
...
static int polygator_radio_channel_power_up(struct radio_channel *channel)
{
    ...
    switch (channel->state) {
        ...
        case RADIO_MODULE_STATE_DISABLED:
            channel->state = RADIO_MODULE_STATE_INIT;
            res = 0;
            break;
        default:
            channel->state = RADIO_MODULE_STATE_FAILED;
...
int main(int argc, char **argv)
{
    ...
    x_sllist_struct_declare(tcp_cc_list, struct tcp_control_client);
    struct tcp_control_client *tcp_cc;
...
//  socklen_t tcp_dc_addrlen;
    uint8_t tcp_dc_recv_buf[0x10000];
    size_t tcp_dc_recv_length = 0;
    size_t tcp_dc_recv_wait = 0;
    uint8_t tcp_dc_xmit_buf[0x10000];
    ...
    size_t sim_cmd_expected_length;
    ...
    while (run) {
        ...
        for (tcp_cc = tcp_cc_list.head; tcp_cc; tcp_cc = tcp_cc->next) {
            // xmit data
            if (tcp_cc->xmit_length) {
                // send data
                if (send(tcp_cc->sock, tcp_cc->xmit_buf, tcp_cc->xmit_length, 0) < 0) {
                    LOG("%s: send(tcp_cc->sock) failed - %s\n", tcp_cc->prefix, strerror(errno));
                    // set close flag
                    tcp_cc->flags.close = 1;
                }
                tcp_cc->xmit_length = 0;
            }
            // flags
            // close
            if (tcp_cc->flags.close) {
                //LOG("%s: Connection with \"%s:%u\" closed\n", tcp_cs_prefix, inet_ntoa(tcp_cc->addr.sin_addr), ntohs(tcp_cc->addr.sin_port));
                // remove list entry
                x_sllist_remove_entry(tcp_cc_list, tcp_cc);
                close(tcp_cc->sock);
                free(tcp_cc);
                break;
            }
            ...
            for (board = board_list.head; board; board = board->next) {
                for (radio_channel = board->radio_channel_list.head; radio_channel; radio_channel = radio_channel->next) {
                    if (radio_channel->flags.run) {
                        ...
                        // stop data timer
                        x_timer_stop(radio_channel->timers.data);
                    }
                }
            }
        }
        ...
        for (board = board_list.head; board; board = board->next) {
            for (radio_channel = board->radio_channel_list.head; radio_channel; radio_channel = radio_channel->next) {
                ...
                if (is_x_timer_enable(radio_channel->timers.wait) && is_x_timer_fired(radio_channel->timers.wait)) {
                    ...
                    sc_data.body.data[0] = 0x60;
                    // dump
                    if ((radio_channel->dump) && (fp = fopen(radio_channel->dump, "a"))) {
                        dumptime(fp);
                        fprintf(fp, "Write data length=%lu\n", (long unsigned int)sc_data.header.length);
                        dumphex(fp, 4, sc_data.body.data, sc_data.header.length);
                        fclose(fp);
                    }
                    do {
                ...
                // data
                if (is_x_timer_enable(radio_channel->timers.data) && is_x_timer_fired(radio_channel->timers.data)) {
                    LOG("%s: DATA timer fired\n", radio_channel->device);
                    x_timer_stop(radio_channel->timers.data);
                    // reset received data buffer from device
                    radio_channel->sim_cmd_length = 0;
                }
                // signals
                // enable
                if (radio_channel->signals.enable) {
                    if (radio_channel->flags.power_down == 0) {
                ...
                if (radio_channel->signals.shutdown) {
                    if (radio_channel->flags.power_up == 0) {
        ...
        for (tcp_cc = tcp_cc_list.head; tcp_cc; tcp_cc = tcp_cc->next) {
            if (tcp_cc->sock > 0) {
                FD_SET(tcp_cc->sock, &rfds);
                maxfd = mmax(tcp_cc->sock, maxfd);
            }
        }
        ...
        if (res > 0) {
            ...
            if ((tcp_cs_sock > 0) && (FD_ISSET(tcp_cs_sock, &rfds))) {
                ...
                if ((res = accept(tcp_cs_sock, (struct sockaddr *)&tcp_rem_addr, &tcp_rem_addrlen)) < 0) {
                    ...
                    if ((tmp_flags = fcntl(res, F_GETFL)) < 0) {
                        ...
                        if ((tcp_cc = calloc(1, sizeof(struct tcp_control_client)))) {
                            // append client to list
                            x_sllist_insert_tail(tcp_cc_list, tcp_cc);
                            // accept new client connection
                            //LOG("%s: Connection from \"%s:%u\" accepted\n", tcp_cs_prefix, inet_ntoa(tcp_rem_addr.sin_addr), ntohs(tcp_rem_addr.sin_port));
                            // set client data
                            tcp_cc->sock = res;
                            memcpy(&tcp_cc->addr, &tcp_rem_addr, tcp_rem_addrlen);
                            tcp_cc->addrlen = tcp_rem_addrlen;
                            snprintf(tcp_cc->prefix, sizeof(tcp_cc->prefix), "Control Client(%s:%u)", inet_ntoa(tcp_cc->addr.sin_addr), ntohs(tcp_cc->addr.sin_port));
                            // init TCP client data buffer
                            tcp_cc->recv_length = 0;
                            tcp_cc->recv_wait = sizeof(struct ss_ctrl_msg_req_hdr);
                            tcp_cc->xmit_length = 0;
                        } else {
                            LOG("%s: Discard connection from \"%s:%u\" - cannot allocate memory for struct tcp_control_client\n", tcp_cs_prefix, inet_ntoa(tcp_rem_addr.sin_addr), ntohs(tcp_rem_addr.sin_port));
                            close(res);
                        }
                    }
                }
            ...
            // TCP control client socket
            for (tcp_cc = tcp_cc_list.head; tcp_cc; tcp_cc = tcp_cc->next) {
                if ((tcp_cc->sock > 0) && (FD_ISSET(tcp_cc->sock, &rfds))) {
                    res = recv(tcp_cc->sock, &tcp_cc->recv_buf[tcp_cc->recv_length], tcp_cc->recv_wait - tcp_cc->recv_length, 0);
                    if (res > 0) {
                        tcp_cc->recv_length += res;
                        // check for request header
                        if (tcp_cc->recv_length >= sizeof(struct ss_ctrl_msg_req_hdr)) {
                            ptr_ss_ctrl_msg_req_hdr = (struct ss_ctrl_msg_req_hdr *)tcp_cc->recv_buf;
                            // correct wait length
                            tcp_cc->recv_wait = sizeof(struct ss_ctrl_msg_req_hdr) + ptr_ss_ctrl_msg_req_hdr->length;
                            // check for full message
                            if (tcp_cc->recv_length >= sizeof(struct ss_ctrl_msg_req_hdr) + ptr_ss_ctrl_msg_req_hdr->length) {
                                // check for match id
                                if (ptr_ss_ctrl_msg_req_hdr->id == id) {
                                    ...
                                    switch (ptr_ss_ctrl_msg_req_hdr->cmd) {
                                        case SS_CTRL_MSG_STATUS:
                                            // prepare response
                                            ptr_ss_ctrl_msg_resp_hdr = (struct ss_ctrl_msg_resp_hdr *)&tcp_cc->xmit_buf[tcp_cc->xmit_length];
                                            ...
                                            tcp_cc->xmit_length += sizeof(struct ss_ctrl_msg_resp_hdr);
                                            for (board = board_list.head; board; board = board->next) {
                                                for (radio_channel = board->radio_channel_list.head; radio_channel; radio_channel = radio_channel->next) {
                                                    // get GSM module vio
                                                    ...
                                                    memcpy(&tcp_cc->xmit_buf[tcp_cc->xmit_length], &status, sizeof(unsigned short));
                                                    tcp_cc->xmit_length += sizeof(unsigned short);
                                                }
                                            }
                                                ...
                                                for (radio_channel = board->radio_channel_list.head; radio_channel; radio_channel = radio_channel->next) {
                                                    if (radio_channel->id == ptr_ss_ctrl_msg_req_hdr->chnl) {
                                                        ...
                                                        for (i = 0; i < ATR_MAXLEN; ++i) {
                                                            if (atr_read_byte(&radio_channel->atr, tcp_cc->recv_buf[sizeof(struct ss_ctrl_msg_req_hdr) + i]) < 0) {
#if 0
                                                                LOG("%s: ENABLE channel=%u failed: bad byte in ATR[%lu]=0x%02x\n", tcp_cc->prefix, ptr_ss_ctrl_msg_req_hdr->chnl, (unsigned long int)i, tcp_cc->recv_buf[sizeof(struct ss_ctrl_msg_req_hdr) + i]);
                                                                radio_channel->atr.length = 0;
                                        ...
                                        case SS_CTRL_MSG_DISABLE:
                                            ...
                                            for (board = board_list.head; board; board = board->next) {
                                                for (radio_channel = board->radio_channel_list.head; radio_channel; radio_channel = radio_channel->next) {
                                                    if (radio_channel->id == ptr_ss_ctrl_msg_req_hdr->chnl) {
                                                        if ((radio_channel->flags.run) || (polygator_radio_channel_get_status(radio_channel->board->path, radio_channel->position) == 1)) {
                                            ...
                                            break;
                                        default:
                                            LOG("%s: Unknown command=0x%02x\n", tcp_cc->prefix, ptr_ss_ctrl_msg_req_hdr->cmd);
                        ...
                        if (errno != EAGAIN) {
                            LOG("%s: recv(tcp_cc->sock) failed - %s\n", tcp_cc->prefix, strerror(errno));
                            // set close flag
                            tcp_cc->flags.close = 1;
                        }
                    } else {
                                                            ...
                                                            if (radio_channel->id == ptr_ss_data_msg_comb_chunk_hdr->chnl) {
                                                                ...
                                                                if (is_x_timer_enable(radio_channel->timers.command)) {
                                                                    ...
                                                                    if (ptr_ss_data_msg_comb_chunk_hdr->length == 2) {
                                                                        ...
                                                                        memcpy(sc_data.body.data, &tcp_dc_recv_buf[i], ptr_ss_data_msg_comb_chunk_hdr->length);
                                                                    } else {
                                                                        sc_data.header.length = ptr_ss_data_msg_comb_chunk_hdr->length + 1;
                                                                        sc_data.body.data[0] = radio_channel->sim_cmd[1];
                                                                        memcpy(&sc_data.body.data[1], &tcp_dc_recv_buf[i], ptr_ss_data_msg_comb_chunk_hdr->length);
                                                                    }
                                                                    do {
                                                                        if (write(radio_channel->sim_data_fd, &sc_data, sizeof(sc_data.header) + sc_data.header.length) < 0) {
                                                                            if (errno != EAGAIN) {
                        ...
                        if (res > 0) {
                            switch (sc_data.header.type) {
                                case SIMCARD_CONTAINER_TYPE_DATA:
                                    ...
                                    if ((radio_channel->dump) && (fp = fopen(radio_channel->dump, "a"))) {
                                        dumptime(fp);
                                        fprintf(fp, "Read data length=%lu\n", (long unsigned int)sc_data.header.length);
                                        dumphex(fp, 4, sc_data.body.data, sc_data.header.length);
                                        fclose(fp);
                                    }
                                    // check for data from device is present
                                    if (sc_data.header.length == 0) {
                                        LOG("%s: received empty message within type=data res=%d\n", radio_channel->device, res);
                                        break;
                                    }
                                    // copy data to processing buffer
                                    memcpy(&radio_channel->sim_cmd[radio_channel->sim_cmd_length], sc_data.body.data, sc_data.header.length);
                                    radio_channel->sim_cmd_length += sc_data.header.length;
                                    // check for expected length of data has received
                                    sim_cmd_expected_length = 1;
                                    if (radio_channel->sim_cmd_length >= sim_cmd_expected_length) {
                                        if (radio_channel->sim_cmd[0] == 0xff) {
                                            sim_cmd_expected_length += 2;
                                            // PPS
                                            if (radio_channel->sim_cmd_length >= sim_cmd_expected_length) {
                                        ...
                                        } else if (radio_channel->sim_cmd[0] == 0x00) {
                                            // CLA=00
                                            sim_cmd_expected_length += 4;
                                            if (radio_channel->sim_cmd_length >= sim_cmd_expected_length) {
                                                if ((radio_channel->sim_cmd[1] != 0x84)
                                                        && (radio_channel->sim_cmd[1] != 0xb0)
                                                        && (radio_channel->sim_cmd[1] != 0xb2)
                                                        && (radio_channel->sim_cmd[1] != 0xc0)
                                                        && (radio_channel->sim_cmd[1] != 0xca)
                                                        && (radio_channel->sim_cmd[4] != 0x00)) {
                                                    sim_cmd_expected_length += radio_channel->sim_cmd[4];
                                                }
                                            }
                                        } else if (radio_channel->sim_cmd[0] == 0x80) {
                                            // CLA=80
                                            sim_cmd_expected_length += 4;
                                            if (radio_channel->sim_cmd_length >= sim_cmd_expected_length) {
                                                if ((radio_channel->sim_cmd[1] != 0xf2)
                                                        && (radio_channel->sim_cmd[1] != 0xcb)
                                                        && (radio_channel->sim_cmd[1] != 0x12)
                                                        && (radio_channel->sim_cmd[4] != 0x00)) {
                                                    sim_cmd_expected_length += radio_channel->sim_cmd[4];
                                                }
                                            }
                                        } else if (radio_channel->sim_cmd[0] == 0xa0) {
                                            // CLA=GSM
                                            sim_cmd_expected_length += 4;
                                            if (radio_channel->sim_cmd_length >= sim_cmd_expected_length) {
                                                if ((radio_channel->sim_cmd[1] != 0x12)
                                                        && (radio_channel->sim_cmd[1] != 0xb0)
                                                        && (radio_channel->sim_cmd[1] != 0xb2)
                                                        && (radio_channel->sim_cmd[1] != 0xc0)
                                                        && (radio_channel->sim_cmd[1] != 0xf2)
                                                        && (radio_channel->sim_cmd[4] != 0x00)) {
                                                    sim_cmd_expected_length += radio_channel->sim_cmd[4];
                                                }
                                            }
                                    ...
                                    if (radio_channel->sim_cmd_length < sim_cmd_expected_length) {
                                        if (radio_channel->sim_cmd_length == 5) {
                                            // write ACK
                                            sc_data.header.type = SIMCARD_CONTAINER_TYPE_DATA;
                                            sc_data.header.length = 1;
                                            sc_data.body.data[0] = radio_channel->sim_cmd[1];
                                            ...
                                            if ((radio_channel->dump) && (fp = fopen(radio_channel->dump, "a"))) {
                                                dumptime(fp);
                                                fprintf(fp, "Write data length=%lu\n", (unsigned long int)sc_data.header.length);
                                                dumphex(fp, 4, sc_data.body.data, sc_data.header.length);
                                                fclose(fp);
                                            }
                                            do {
                                        ...
                                        x_timer_set_second(radio_channel->timers.data, 1);
                                        break;
                                    }
                                    x_timer_stop(radio_channel->timers.data);
                                    // processing received data
                                    if (radio_channel->sim_cmd[0] == 0xff) { // PPS
                                        // log
                                        if ((radio_channel->log) && (fp = fopen(radio_channel->log, "a"))) {
                                            dumptime(fp);
                                            fprintf(fp, "PPS request length=%lu\n", (unsigned long int)sc_data.header.length);
                                            dumphex(fp, 4, sc_data.body.data, sc_data.header.length);
                                            fclose(fp);
                                        }
                                        // write PPS response
                                        sc_data.header.type = SIMCARD_CONTAINER_TYPE_DATA;
                                        sc_data.header.length = radio_channel->sim_cmd_length;
                                        memcpy(sc_data.body.data, radio_channel->sim_cmd, radio_channel->sim_cmd_length);
                                        // dump
                                        if ((radio_channel->dump) && (fp = fopen(radio_channel->dump, "a"))) {
                                            dumptime(fp);
                                            fprintf(fp, "Write data length=%lu\n", (unsigned long int)sc_data.header.length);
                                        ...
                                        if ((radio_channel->log) && (fp = fopen(radio_channel->log, "a"))) {
                                            dumptime(fp);
                                            fprintf(fp, "PPS response length=%lu\n", (unsigned long int)sc_data.header.length);
                                            dumphex(fp, 4, sc_data.body.data, sc_data.header.length);
                                            fclose(fp);
                                        }
                                        // apply data rate
                                        if (radio_channel->sim_cmd[1] & 0x10) {
                                            ...
                                            sc_data.header.length = sizeof(sc_data.body.speed);
                                            if (radio_channel->sim_cmd[2] == 0x96) {
                                                sc_data.body.speed = 0x96;
                                            } else if (radio_channel->sim_cmd[2] == 0x95) {
                                                sc_data.body.speed = 0x95;
                                            } else if (radio_channel->sim_cmd[2] == 0x94) {
                                                sc_data.body.speed = 0x94;
                                            } else {
                                                sc_data.body.speed = 0x11;
                                            }
                                            do {
                                                if (write(radio_channel->sim_data_fd, &sc_data, sizeof(sc_data.header) + sc_data.header.length) < 0) {
                                                    if (errno != EAGAIN) {
                                        ...
                                        LOG("%s: unknown/unsupported SIM-command class=%02x\n", radio_channel->device, radio_channel->sim_cmd[0]);
                                        // reset command index
                                        radio_channel->sim_cmd_length = 0;
                                        // stop wait timer
                                        x_timer_stop(radio_channel->timers.wait);
                                ...
                                case SIMCARD_CONTAINER_TYPE_RESET:
                                    LOG("%s: RESET %s\n", radio_channel->device, sc_data.body.reset ? "high" : "low (active)");
                                    // dump
                                    if ((radio_channel->dump) && (fp = fopen(radio_channel->dump, "a"))) {
                                        dumptime(fp);
                                        fprintf(fp, "RESET %s\n", sc_data.body.reset ? "high" : "low (active)");
                                        fclose(fp);
                                    }
                                    // log
                                    if ((radio_channel->log) && (fp = fopen(radio_channel->log, "a"))) {
                                    ...
                                    if (sc_data.body.reset) {
                                        ...
                                        if (radio_channel->atr.length) {
                                            ...
                                            memcpy(sc_data.body.data, radio_channel->atr.data, radio_channel->atr.length);
                                            do {
                                                if (write(radio_channel->sim_data_fd, &sc_data, sizeof(sc_data.header) + sc_data.header.length) < 0) {
                                                    if (errno != EAGAIN) {
                                                        LOG("%s: write(sim_data_fd): %s\n", radio_channel->device, strerror(errno));
...
main_end:
    ...
    close(tcp_cs_sock);
    while ((tcp_cc = x_sllist_remove_head(tcp_cc_list))) {
        close(tcp_cc->sock);
        free(tcp_cc);
    }
    // stop SIM-data control server
    close(tcp_ds_sock);
    // destroy board list
