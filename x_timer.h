...
do { \
	...
	_timer.expires = tv_add(_timer.start, _timer.timeout); \
} while(0)

#define x_timer_schedule(_timer, _expired) \
do { \
	struct timespec __curr_time; \
	clock_gettime(CLOCK_MONOTONIC, &__curr_time); \
	_timer.enable = 1; \
	tv_cpy(_timer.start, __curr_time); \
	_timer.timeout = tv_sub(_expired, __curr_time); \
	tv_cpy(_timer.expires, _expired); \
} while(0)
...
#define get_x_timer_expires(_timer) \
({ \
	struct timespec __res; \
	if (_timer.enable) { \
		tv_cpy(__res, _timer.expires); \
	} else { \
		tv_set(__res, 0, 0); \
	} \
	__res; \
